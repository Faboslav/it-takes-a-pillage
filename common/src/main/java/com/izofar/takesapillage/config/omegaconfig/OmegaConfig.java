package com.izofar.takesapillage.config.omegaconfig;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.izofar.takesapillage.ItTakesPillage;
import com.izofar.takesapillage.config.ConfigDirectory;
...
public final class OmegaConfig
{
		...
		try {
			...
			if (!configExists(config)) {
				...
				try {
					...
					ItTakesPillage.getLogger().error(e.toString());
					ItTakesPillage.getLogger().info(String.format("Encountered an error while reading %s config, falling back to default values.", config.getName()));
					ItTakesPillage.getLogger().info(String.format("If this problem persists, delete the config file %s and try again.", config.getName() + "." + config.getExtension()));
				}
			}
	...
	public static <T extends Config> void writeConfig(Class<T> configClass, T instance) {
		...
		try {
			...
			ItTakesPillage.getLogger().error(ioException.toString());
			ItTakesPillage.getLogger().info(String.format("Write error, using default values for config %s.", configClass));
		}
	}
