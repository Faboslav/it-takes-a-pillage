...
{
	public static Screen createConfigScreen(ItTakesPillageConfig config, Screen parent) {
			...
			for (Field field : config.getClass().getDeclaredFields()) {
				...
				if (field.getGenericType() == boolean.class) {
					category.addEntry(builder.entryBuilder()
						...
						.setDefaultValue(field.getBoolean(new ItTakesPillageConfig()))
					...
					category.addEntry(builder.entryBuilder()
						...
						.setDefaultValue(field.getInt(new ItTakesPillageConfig()))
						.setSaveConsumer((enabled) -> {
							try {
								field.set(config, enabled);
							} catch (IllegalAccessException e) {
